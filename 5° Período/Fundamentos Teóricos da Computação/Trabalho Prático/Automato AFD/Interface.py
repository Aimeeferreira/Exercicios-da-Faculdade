...
while True:
    ...
    if eventos == 'Enviar':# Verifica qual o botão que foi selecionado
        ...
        if(arquivo[-1] == 'txt'): # Verifica se a ultima prosição da minha lista equivale a txt
            ...
            print('Extensão invalida. Somente extensões .txt')#Caso o arquivo não seja txt ele pede para informar uma extensão TXT
            sg.popup('Extensão invalida. Somente extensões .txt')#Aparece um popup para informar uma extensão valida
    else:   
        break
